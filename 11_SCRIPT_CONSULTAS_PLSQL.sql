...
-- Alumno 4: Pablo Alvarez Alvarez
...
DELIMITER $$;
DROP FUNCTION IF EXISTS numero_de_pases $$;
CREATE FUNCTION numero_de_pases ()
RETURNS text
DETERMINISTIC
BEGIN
DECLARE lista text DEFAULT '';
DECLARE nombreJugador varchar(20) default '';
DECLARE fin BOOL default FALSE;
DECLARE c1 CURSOR FOR SELECT Nombre FROM Jugadores, Estadisticas WHERE Jugadores.ID_Jugador = Estadisticas.ID_Jugador AND Pases > 1500;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET fin=TRUE;
OPEN c1;
etiqueta: LOOP
FETCH c1 INTO nombreJugador;
IF fin THEN
LEAVE etiqueta;
END IF;
SET lista = CONCAT(lista, ' , ', nombreJugador);
END LOOP etiqueta;
CLOSE c1;
RETURN(lista);
END $$;
...
SELECT numero_de_pases() as "Jugadores con mas pases de 1500";

-- Ignacio Charlo Millan
-- PLSQL
DELIMITER $$;
DROP FUNCTION IF EXISTS numPorteriasACero $$;
CREATE FUNCTION numPorteriasACero()
RETURNS text
DETERMINISTIC
BEGIN
    DECLARE lista text DEFAULT '';
    DECLARE portero text default '';  
    DECLARE fin BOOL default FALSE;
    DECLARE c1 CURSOR FOR SELECT CONCAT(jugadores.Nombre, " ", jugadores.Apellido, " = ", estadisticas.Porteria_Cero, " || ") FROM jugadores JOIN estadisticas
ON jugadores.ID_Jugador=estadisticas.ID_Jugador WHERE estadisticas.Porteria_Cero >= 8 ;

    DECLARE CONTINUE HANDLER FOR NOT FOUND SET fin=TRUE;
    OPEN c1;
    etiqueta: LOOP
        FETCH c1 INTO portero;
        IF fin THEN
            LEAVE etiqueta;
        END IF;
        SET lista = CONCAT(lista, ' ', portero);
    END LOOP etiqueta;
    CLOSE c1;
    RETURN(lista);
END $$;
...
SELECT numPorteriasACero() as "Porteros con mas de 8 porterias a cero";
